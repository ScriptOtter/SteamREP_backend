...
export class SteamPrismaService {
  ...
  public async findSteamUser(id: string): Promise<Partial<any> | null> {
    try {
      ...
      if (!steamUser) {
        ...
        const lastUpdatedDate = steamUser.lastUpdateSteamInformation; // Получаем дату последнего обновления
        const currentDate = new Date(); // Текущая дата
        console.log(lastUpdatedDate.toDateString(), currentDate.toDateString());
  ...
  public async updateSteamUserFromSteam(
    ...
    try {
      console.log('updateSteamUserFromSteam');
      const steamUser = await this.prisma.steamUser.findUnique({
        where: { id: steamid },
      });
      ...
      if (
        currentDate.toDateString() !==
        steamUser.lastUpdateSteamInformation.toDateString()
      ) {
        ...
        await this.prisma.steamUser.update({
          ...
          data: {
            ...
            realname: realname,
            countryCode: loccountrycode,
            lastUpdateSteamInformation: currentDate,
          },
        });
      }
      return null;
  ...
  public async getViewers(steamid: string): Promise<number> {
    try {
      const steamUser = await this.prisma.steamUser.findFirstOrThrow({
        where: { id: steamid },
      });
      return steamUser.viewers;
    } catch (e) {
